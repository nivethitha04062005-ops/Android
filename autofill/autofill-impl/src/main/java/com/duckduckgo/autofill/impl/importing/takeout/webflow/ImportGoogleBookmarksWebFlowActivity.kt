...
class ImportGoogleBookmarksWebFlowActivity :
    ...
    override fun onWebViewVisibilityChange(shouldShowWebView: Boolean) {
        logcat { "Bookmark-import: ${javaClass.simpleName}, should show web view: $shouldShowWebView" }
    ...
    override fun onCreateOptionsMenu(menu: Menu): Boolean {
        if (appBuildConfig.isInternalBuild()) {
            menuInflater.inflate(R.menu.menu_bookmark_import_debug, menu)
            return true
        }
    ...
    override fun onPrepareOptionsMenu(menu: Menu): Boolean {
        ...
        val debugMenuItem = menu.findItem(R.id.debug_toggle_webview)
        debugMenuItem?.isVisible = canShowDebugMenu
        return true
    }

    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            R.id.debug_toggle_webview -> {
                toggleOverlay()
                true
            }
    ...
    private fun toggleOverlay() {
        val progressFragment = supportFragmentManager.findFragmentByTag(PROGRESS_OVERLAY_TAG)
        if (progressFragment != null) {
            logcat { "ImportGoogleBookmarksWebFlowActivity: Debug menu hiding progress overlay" }
            hideProgressOverlay()
        } else {
            logcat { "ImportGoogleBookmarksWebFlowActivity: Debug menu showing progress overlay" }
            showProgressOverlay()
        }
    }
